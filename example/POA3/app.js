...
(function() {
  ...
  const elements = {
    ...
    coverSpeaker: document.getElementById('coverSpeaker'),
    coverTeam: document.getElementById('coverTeam'),
    coverSpeakerRow: document.getElementById('coverSpeakerRow'),
    coverTeamRow: document.getElementById('coverTeamRow'),
    ...
    keyPointsTitle: document.getElementById('keyPointsTitle'),
    keyPointsIcon: document.getElementById('keyPointsIcon'),
  ...
  function init() {
    ...
    if (typeof hasCover !== 'undefined' && hasCover) {
      // Set cover content
      if (elements.coverTitle) {
        elements.coverTitle.textContent = coverTitle || 'Presentation';
      }
      if (elements.coverSpeaker && coverSpeaker) {
        elements.coverSpeaker.textContent = coverSpeaker;
        if (elements.coverSpeakerRow) {
          elements.coverSpeakerRow.style.display = 'flex';
        }
      } else if (elements.coverSpeakerRow) {
        elements.coverSpeakerRow.style.display = 'none';
      }
      if (elements.coverTeam && coverTeam) {
        elements.coverTeam.textContent = coverTeam;
        if (elements.coverTeamRow) {
          elements.coverTeamRow.style.display = 'flex';
        }
      } else if (elements.coverTeamRow) {
        elements.coverTeamRow.style.display = 'none';
      }

      // Bind start button
      if (elements.coverStartBtn) {
        elements.coverStartBtn.addEventListener('click', function() {
          elements.coverSlide.classList.add('hidden');
        });
      }

      // Show cover slide
      if (elements.coverSlide) {
        elements.coverSlide.classList.remove('hidden');
      }
    } else {
      // No cover slide, show main content
      if (elements.coverSlide) {
        elements.coverSlide.classList.add('hidden');
      }
    }
  ...
  function goToStep(index) {
    ...
    // Render key points
    renderKeyPoints(step.keyPoints);
  ...
  function renderKeyPoints(keyPoints) {
    if (!keyPoints || !keyPoints.items || keyPoints.items.length === 0) {
    ...
    // Set label and icon
    elements.keyPointsTitle.textContent = keyPoints.label || 'Key Points';
    elements.keyPointsIcon.textContent = keyPoints.icon || '💡';

    // Render items
    elements.keyPointsList.innerHTML = keyPoints.items.map(point =>
